...
test("scoped chain-throw: filter → map → map → toPromise, throw in map propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
...
test("scoped chain-throw: mapAsync → distinct → take → connect, throw in mapAsync propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: skip → group → connect, throw in connect propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: filter → pair → map → toPromise, throw in pair map propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: count → filter → map → toPromise, throw in final map propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: skip → take → connect, async throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: distinct → mapAsync → tap → toPromise, throw in mapAsync propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: filter → retry → map → connect, async throw propagates through retry", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: filter → pair → map → toPromise, throw in moving-avg map propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: group → mapAsync → toPromise, throw in mapAsync propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: map → distinct → count → take → connect, async throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
    s.map(e => e.type)
...
test("scoped chain-throw: skip → distinct → mapAsync → toPromise, throw in mapAsync propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: filter → pair → tap → filter → map → toPromise, throw in final map propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: group → mapAsync → tap → toPromise, throw in mapAsync propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: map → retry → skip → map → toPromise, throw in final map propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
    const p = s.map(id => ({ id }))
...
test("scoped chain-throw: distinct → take → mapAsync → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: strideTricks → map → toPromise, throw in map propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: map → distinct → count → filter → map → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: skip → group → mapAsync → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: map → pair → filter → map → toPromise, throw in improvement map propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: distinct → mapAsync → tap → take → toPromise, throw in mapAsync propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: map → filter → mapAsync → toPromise, throw in mapAsync propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: filter → map → pair → map → toPromise, throw in duration map propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: distinct → take → mapAsync → tap → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: group → skip → map → toPromise, throw in map propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: filter → mapAsync → count → take → toPromise, throw in mapAsync propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: map → retry → skip → map → toPromise, throw after skip propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: pair → map → filter → mapAsync → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: filter → distinct → take → connect, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
    s.filter(v => v.trim() !== '')
...
test("scoped chain-throw: group → mapAsync → tap → connect, async throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: fromArray filter → map → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    try {
...
test("scoped chain-throw: fromArray group → map → toPromise, throw in map propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    try {
...
test("scoped chain-throw: fromArray skip → pair → map → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    try {
...
test("scoped chain-throw: fromArray distinct → count → map → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    try {
...
test("scoped chain-throw: fromValue split → filter → map → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    try {
...
test("scoped chain-throw: fromArray distinct → skip → map → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    try {
...
test("scoped chain-throw: fromArray map → distinct → group → map → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    try {
...
test("scoped chain-throw: fromArray reduce → distinct → map → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    try {
...
test("scoped chain-throw: fromValue strideTricks → map → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    try {
...
test("scoped chain-throw: ETL — map → filter → distinct → group → mapAsync → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: ML — map → pair → map → skip → take → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: event sourcing — filter → group → mapAsync → tap → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: IoT — distinct → map → skip → pair → map → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: recommendation — map → filter → distinct → count → take → connect, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: tap at multiple stages — throw in first tap propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
...
test("scoped chain-throw: tap at multiple stages — throw in second tap propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
    const p = s.tap(() => {})
...
test("scoped chain-throw: fromArray full pipeline — filter → distinct → group → mapAsync → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    try {
...
test("scoped chain-throw: subject map → retry(0) → tap → reduce → toPromise, throw in tap propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
    const p = s.map(v => v * 2)
...
test("scoped chain-throw: pair → filter spike → mapAsync → toPromise, async throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
    const p = s.operator(Operator.pair())
...
test("scoped chain-throw: skip → take → distinct → group → map → toPromise, throw propagates", async (t) => {
    process.on("unhandledRejection", (reason) => { t.fail("unhandled rejection: " + reason); });
    process.on("error", (reason) => { t.fail("unhandled error: " + reason); });
    const s = new Subject();
    const p = s.operator(Operator.skip(1))
